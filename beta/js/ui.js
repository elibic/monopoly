...
(function () {
  ...
  function narrationFor(g, entry) {
    ...
    switch (entry.kind) {
      case 'turn':
        ...
        if (t.includes('הקופה')) return null; // בונוס הקופה מוקרא במלואו
        if (!vk) return null;
        return [`ev_turn_${vk}`];
      case 'buy':
  ...
  const TILE_INFO = {
    ...
    parking: ['חניה חופשית 🅿️', 'משבצת מנוחה — חונים ונחים עד התור הבא. אבל אם הצטברה כאן קופה — הנוחת זוכה בכולה וגם מטיל שוב!'],
